...
class VoyagerClient:
    ...
    def _request(self, path: str, method: str = "GET", body: Optional[dict] = None) -> Any:
        # FastMCP runs on asyncio; sync_playwright() raises if called on the event
        # loop thread.  Run the browser work in a plain worker thread instead.
    ...
    def _browser_request(self, path: str, method: str, body: Optional[dict]) -> Any:
        ...
        # Resource types that contribute nothing to the API call.
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # Block images, fonts, media — only HTML/JS/XHR needed for CORS origin.
                page.route(
                    "**/*",
                ...
                # Navigate to /feed/ so LinkedIn's JS runs and refreshes the CSRF
                # token.  Resources are blocked above so only HTML/JS loads.
                page.goto(
                    "https://www.linkedin.com/feed/",
                    wait_until="domcontentloaded",
                    timeout=self._timeout_ms,
                )
                # Read the live JSESSIONID after navigation — LinkedIn may rotate it.
    ...
    def _browser_scrape_notifications(self, count: int) -> list[dict]:
        ...
        _EXTRACT_SCRIPT = """
            () => {
                ...
                // Fallback: return the full main text split into paragraphs
    ...
    def get_conversations(self, entity_urn: str, count: int = 20) -> list[dict]:
        ...
        """
        # Normalise to fsd_profile: the raw ID is the last colon-separated token.
        ...
        raw = self._get(path)
        # Response uses Voyager normalisation: conversations are in included[]
        included = raw.get("included", [])
        convos = [
            ...
            try:
                ...
                for section, path in pages.items():
                    try:
                        ...
                        # Scroll to trigger lazy-loaded section content, then settle.
                        page.evaluate("window.scrollTo(0, 600)")
